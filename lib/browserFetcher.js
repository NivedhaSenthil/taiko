...
const preferredRevision = require(path.join(helper.projectRoot(), 'package.json')).taiko.chromium_revision;
var url = require('url');
...
class BrowserFetcher {
    ...
    _resolveExecutablePath() {
        ...
        if (executablePath) {
            const missingText = !fs.existsSync(executablePath) ? 'Tried to use TAIKO_BROWSER_PATH env variable to launch browser but did not find any executable at: ' + executablePath : null;
            return { executablePath, missingText };
        }
        const revisionInfo = this.revisionInfo(preferredRevision);
        console.log('@@@@@@@@@@@@@@@@@@@@@@@@@@@');
        console.log(revisionInfo);
        const missingText = !revisionInfo.local ? 'Chromium revision is not downloaded. Provide TAIKO_BROWSER_PATH or Install taiko again to download bundled chromium.' : null;
    ...
    waitForWSEndpoint(chromeProcess, timeout) {
        return new Promise((resolve, reject) => {
            ...
            function onClose(error) {
                cleanup();
                reject(new Error('Failed to launch chrome!' + (error ? ' ' + error.message : '') + '\n' + stderr));
            }
            ...
            function onLine(line) {
                ...
                if (!match)
                    return;
                cleanup();
                const endpoint = { host: url.parse(match[1]).hostname, port: url.parse(match[1]).port };
                resolve(endpoint);
            }
...
function downloadFile(url, destinationPath, progressCallback) {
    ...
    const promise = new Promise((x, y) => { fulfill = x;
        reject = y; });
